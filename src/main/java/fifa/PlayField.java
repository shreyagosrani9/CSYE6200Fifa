...
public class PlayField {
	...
	public PlayField(Elements list, Color[] colors) {
		...
		Arc rightArc = new Arc(midpointX(W2 + H2, W2 + H4), midpointY(H2, H2 - SQ2), H5, H5, 120, WIDTH / 6.7);
		rightArc.setFill(Color.TRANSPARENT);
		rightArc.setStroke(Color.WHITE);
		rightArc.setStrokeWidth(2);
		rightArc.setLength(ARCLENGTH);

		Arc downArc = new Arc(midpointX(W2 + H4, W2 - H4), H2 + (H2 * sqrt(3) / 2), H5, H5, 0, WIDTH / 6.7);
	...
	private Polyline createGoal(GoalType type, Color[] colors) {
		...
		case BLUE:
			goal.getPoints().addAll(midpointX(W2 - H2, midpointX(W2 - H2, W2 - H4)) + HEIGHT / 20,
					...
					midpointX(W2 - H2, midpointX(W2 - H2, W2 - H4)), midpointY(H2, midpointY(H2, H2 - SQ2)),
					midpointX(W2 - H4, midpointX(W2 - H2, W2 - H4)), midpointY(H2 - SQ2, midpointY(H2, H2 - SQ2)),
					midpointX(W2 - H4, midpointX(W2 - H2, W2 - H4)) + HEIGHT / 20,
					midpointY(H2 - SQ2, midpointY(H2, H2 - SQ2)) + HEIGHT / 40);
		...
		case RED:
			goal.getPoints().addAll(midpointX(W2 + H4, midpointX(W2 + H4, W2 - H4)),
					...
					H2 + (H2 * sqrt(3) / 2), midpointX(W2 - H4, midpointX(W2 + H4, W2 - H4)), H2 + (H2 * sqrt(3) / 2),
					midpointX(W2 - H4, midpointX(W2 + H4, W2 - H4)), H2 + (H2 * sqrt(3) / 2) - HEIGHT / 20);
