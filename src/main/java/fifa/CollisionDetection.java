...
public class CollisionDetection {
    ...
    private void resolveBallCollision(Ball particle, Ball otherParticle) {
        ...
        double xDist = otherParticle.position.x - particle.position.x;
        double yDist = otherParticle.position.y - particle.position.y;

        // Prevent accidental overlap of particles
        if (xVelocityDiff * xDist + yVelocityDiff * yDist >= 0) {

            // Grab angle between the two colliding particles
            double angle = -Math.atan2(otherParticle.position.y - particle.position.y, otherParticle.position.x - particle.position.x);
